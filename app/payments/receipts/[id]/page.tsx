...
export default async function PaymentReceiptPage({params}:{params:Promise<{id:string}>}){
  ...
  const [{data:customer},{data:order},{data:billingProfile}]=await Promise.all([
    ...
    attempt.local_order_id?supabase.from("marketplace_orders").select("amount,service_title,marketplace_services(title)").eq("id",attempt.local_order_id).maybeSingle():Promise.resolve({data:null}),
  ...
  const baseAmount=attempt.target_type==="resume_download"?getResumeTemplate(attempt.target_id).price:order?.amount?Number(order.amount):null;
  const processingRecovery=baseAmount!==null?Math.max(0,Number(attempt.amount)-baseAmount):0;
  ...
  return <main className="min-h-screen bg-[#f5f5f3] px-5 pb-24 pt-36 print:bg-white print:p-0 sm:px-8"><div className="mx-auto max-w-4xl">
    ...
    <article className="overflow-hidden rounded-[2.5rem] border border-zinc-200 bg-white shadow-xl print:rounded-none print:border-0 print:shadow-none">
      <header className="flex flex-wrap items-start justify-between gap-6 bg-zinc-950 p-8 text-white sm:p-12 print:bg-white print:px-0 print:text-zinc-950"><div className="flex items-center gap-4"><div className="rounded-2xl bg-white p-2"><Image src="/images/branding/jobiverse-logo.webp" alt="JobiVerse" width={54} height={54}/></div><div><h1 className="text-2xl font-bold">JobiVerse</h1><p className="mt-1 text-sm text-zinc-400">India&apos;s Hiring & Career Platform</p><p className="mt-1 text-xs text-zinc-400">GSTIN: {JOBIVERSE_GSTIN}</p></div></div><div className="text-right"><p className="text-xs font-bold uppercase tracking-[.18em] text-zinc-400">Payment Receipt</p><p className="mt-2 font-mono text-sm">{receiptNumber}</p></div></header>
      <div className="p-8 sm:p-12 print:px-0"><div className="flex items-center gap-3 rounded-2xl bg-emerald-50 p-5 text-emerald-800"><BadgeCheck/><div><p className="font-bold">Payment successfully received</p><p className="mt-1 text-sm">Verified through Razorpay</p></div></div>
        ...
        <section className="mt-8 overflow-hidden rounded-2xl border border-zinc-200"><div className="grid grid-cols-[1fr_auto] gap-4 bg-zinc-50 px-5 py-4 text-xs font-bold uppercase tracking-wider text-zinc-500"><span>Description</span><span>Amount</span></div><div className="grid grid-cols-[1fr_auto] gap-4 px-5 py-6"><div><p className="font-semibold">{description}</p><p className="mt-2 text-xs text-zinc-500">{attempt.target_type.replaceAll("_"," ")}</p></div><p className="font-bold">{money(baseAmount??attempt.amount)}</p></div>{baseAmount!==null&&processingRecovery>0&&<div className="grid grid-cols-[1fr_auto] gap-4 border-t border-zinc-100 px-5 py-4 text-sm"><span className="text-zinc-500">Payment processing recovery</span><span className="font-semibold">{money(processingRecovery)}</span></div>}<div className="flex justify-between border-t border-zinc-200 bg-zinc-950 px-5 py-5 text-white print:bg-zinc-100 print:text-zinc-950"><span className="font-semibold">Total paid</span><span className="text-xl font-bold">{money(attempt.amount)}</span></div></section>
