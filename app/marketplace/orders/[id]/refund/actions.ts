...
export async function requestOrderRefund(formData:FormData){
  ...
  const{data:attempt}=await supabase.from("payment_attempts").select("id,amount").eq("local_order_id",order.id).eq("status","captured").maybeSingle();if(!attempt)throw new Error("Captured payment record not found.");
  const{error}=await supabase.from("marketplace_refund_requests").insert({order_id:order.id,customer_id:user.id,payment_attempt_id:attempt.id,amount:attempt.amount,reason});if(error)throw new Error(error.code==="23505"?"A refund request already exists for this order.":error.message);
