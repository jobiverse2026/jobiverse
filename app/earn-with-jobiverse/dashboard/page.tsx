...
type DashboardProps = { searchParams: Promise<{ published?: string; updated?: string }> };
...
export default async function CreatorDashboardPage({ searchParams }: DashboardProps) {
  ...
  const [{ data: listings }, { data: orders }] = await Promise.all([
    supabase.from("marketplace_services").select("id,title,slug,audience,price,status,total_orders,view_count,created_at").eq("provider_id", user.id).order("created_at", { ascending: false }),
