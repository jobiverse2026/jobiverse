...
export default async function FeatureServicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: service }, { data: billingProfile }] = await Promise.all([
    supabase.from("marketplace_services").select("id,title,short_description,price,status,is_featured,featured_until").eq("id", id).eq("provider_id", user.id).maybeSingle(),
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
          <section className="relative overflow-hidden rounded-[2.75rem] bg-zinc-950 p-8 text-white sm:p-12">
            ...
            <p className="relative z-10 mt-5 text-xs font-bold uppercase tracking-[.18em] text-violet-300">Paid featured placement</p>
            <h1 className="relative z-10 mt-3 text-4xl font-semibold tracking-[-.04em]">Put this service in the featured section.</h1>
            <p className="relative z-10 mt-4 max-w-2xl text-zinc-400">Featured listings get premium placement in marketplace discovery for 30 days. Publishing remains free; featured placement is optional and paid.</p>
            <div className="relative z-10 mt-8 rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <p className="mt-3 text-sm leading-6 text-zinc-400">{service.short_description}</p>
            </div>
          </section>
          <aside className="h-fit rounded-[2.5rem] border border-zinc-200 bg-white p-8 shadow-xl">
            ...
            <p className="mt-3 text-sm leading-6 text-zinc-500">Your customer-facing service price stays separate. This fee only promotes your listing.</p>
            ...
            {activeFeatured ? (
              ...
              <div className="mt-6"><RazorpayPaymentButton targetType="featured_listing" targetId={service.id} label={`Pay ₹${amount.toLocaleString("en-IN")} & Feature`} /></div>
            ) : (
              <p className="mt-6 rounded-xl bg-amber-50 p-4 text-sm font-semibold text-amber-800">Save your billing profile below to enable payment.</p>
            )}
          </aside>
        </div>
        <div className="mt-6"><BillingProfileCard profile={billingProfile} /></div>
      </div>
    </main>
  );
}
