...
import { ArrowRight, BarChart3, BellRing, Bookmark, BrainCircuit, BriefcaseBusiness, CalendarDays, CircleDollarSign, FileText, Gift, MapPin, Search, Sparkles, Target, UserRound } from "lucide-react";
...
export default async function CandidateDashboardPage({ searchParams }: { searchParams: Promise<{ visibility?: string }> }) {
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[#f5f5f3] px-5 pb-24 pt-36 sm:px-8"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_12%_10%,white,transparent_28%),radial-gradient(circle_at_88%_22%,rgba(99,102,241,.11),transparent_24%)]" /><div className="relative mx-auto max-w-7xl">
    ...
    <section className="mt-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-zinc-400">Talent tools</p><h2 className="mt-2 text-3xl font-semibold tracking-[-.035em]">Manage your career workspace.</h2><div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3"><Action href="/candidates/profile" title="Professional Profile" description="Skills, experience and career preferences" icon={UserRound} /><Action href="/candidates/resume" title="Resume Studio & Services" description="Upload, replace and view your CV | Premium templates coming soon" icon={FileText} /><Action href="/candidates/resume-analysis" title={aiEnabled ? "AI Resume Analyzer" : "AI Resume Analyzer | Coming Soon"} description={aiEnabled ? "Run AI-powered ATS, keyword and impact analysis on your uploaded CV" : "ATS estimate and actionable CV intelligence will unlock when AI credits are active"} icon={BrainCircuit} badge={aiEnabled ? "AI live" : "AI soon"} /><Action href="/candidates/interview-prep" title="Interview Preparation" description="Free readiness checklist with a Career Plus answer notebook" icon={Sparkles} badge="Freemium" /><Action href="/candidates/jobs" title="Explore Opportunities" description="Discover published roles matching your goals" icon={BriefcaseBusiness} /><Action href="/candidates/card-analytics" title="JobiVerse Card Analytics" description="Track public views and visibility of your career card" icon={BarChart3} /><Action href="/candidates/job-alerts" title="Job Alerts" description="Save role, location and work-mode preferences" icon={BellRing} /><Action href="/candidates/saved-jobs" title="Saved Jobs" description="Review saved opportunities and continue applications" icon={Bookmark}/><Action href="/candidates/applications" title="Application Health" description="Track submitted applications, interviews and offers" icon={CalendarDays}/><Action href="/earn-with-jobiverse/dashboard" title="Earn with JobiVerse" description="Create offerings and track sales, earnings and payouts" icon={CircleDollarSign} /></div></section>
  </div></main>;
}

function Action({ href, title, description, icon: Icon, badge }: { href: string; title: string; description: string; icon: typeof UserRound; badge?: string }) {
  return <Link href={href} className="group rounded-3xl border border-white bg-white/90 p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="flex items-start justify-between gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-zinc-950 text-white"><Icon size={21} /></span>{badge&&<span className="rounded-full bg-zinc-100 px-3 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-zinc-600">{badge}</span>}</div><h2 className="mt-6 text-xl font-semibold">{title}</h2><p className="mt-2 text-sm leading-6 text-zinc-500">{description}</p><span className="mt-6 flex items-center gap-2 text-sm font-semibold">Open <ArrowRight size={15} className="transition group-hover:translate-x-1" /></span></Link>;
}
