...
import { cancelEmployerRecruiterInvite, inviteEmployerRecruiter, updateEmployerTeamMemberStatus } from "./actions";

export default async function EmployerTeamPage({ searchParams }: { searchParams: Promise<{ invited?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f5f5f3] px-5 pb-24 pt-36 sm:px-8"><div className="mx-auto max-w-7xl">
    ...
    {!company ? <section className="mt-7 rounded-[2rem] border border-dashed bg-white p-14 text-center"><h2 className="text-2xl font-semibold">Create company profile first</h2><Link href="/employers/company" className="mt-5 inline-flex rounded-xl bg-zinc-950 px-5 py-3 font-semibold text-white">Open company profile</Link></section> : <>
      {params.invited && <p className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-700">Invitation created for {params.invited}. If SMTP invite is delayed, ask them to signup/login with the same email and use the invite link from this page when shared.</p>}
      <section className="mt-7 grid gap-5 md:grid-cols-3"><Metric label="Seat limit" value={company.recruiter_seat_limit}/><Metric label="Used seats" value={used}/><Metric label="Available" value={Math.max(0, company.recruiter_seat_limit - used)}/></section>
      <section className="mt-7 grid gap-7 lg:grid-cols-[.9fr_1.1fr]">
        <form action={inviteEmployerRecruiter} className="rounded-[2rem] border bg-white p-7 shadow-sm"><Mail className="text-zinc-400"/><h2 className="mt-5 text-2xl font-semibold">Invite by email</h2><p className="mt-2 text-sm leading-6 text-zinc-500">Only this exact email can accept the invite. Seat limit is controlled by JobiVerse admin.</p><input name="email" type="email" required placeholder="recruiter@company.com" className="mt-5 h-13 w-full rounded-xl border border-zinc-200 bg-zinc-50 px-4 outline-none focus:border-zinc-500"/><button disabled={used >= company.recruiter_seat_limit} className="mt-4 w-full cursor-pointer rounded-xl bg-zinc-950 px-5 py-3 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">Send invite</button></form>
        <section className="rounded-[2rem] border bg-white p-7 shadow-sm"><div className="flex items-center gap-3"><UsersRound/><div><h2 className="text-2xl font-semibold">Active team</h2><p className="text-sm text-zinc-500">Company recruiters working under {company.company_name}.</p></div></div><div className="mt-5 space-y-3">{members?.length ? members.map((member:any)=>{const person=Array.isArray(member.users)?member.users[0]:member.users;return <article key={member.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-zinc-50 p-4"><div><p className="font-semibold">{person?.full_name || member.email}</p><p className="text-xs text-zinc-500">{member.email} | {member.role}</p></div><form action={updateEmployerTeamMemberStatus}><input type="hidden" name="memberId" value={member.id}/><button name="status" value={member.status==="active"?"disabled":"active"} className={`cursor-pointer rounded-xl px-4 py-2 text-xs font-semibold ${member.status==="active"?"bg-red-50 text-red-700":"bg-emerald-50 text-emerald-700"}`}>{member.status==="active"?"Disable":"Enable"}</button></form></article>}) : <Empty text="No active company recruiters yet."/>}</div></section>
      </section>
