...
import { JoviGuide } from "@/components/brand/jovi-guide";
import { ProfileProgressRing } from "@/components/motion/profile-progress-ring";
...
export default async function OnboardingPage() {
  ...
  return <main className="min-h-screen bg-[#f5f5f3] px-5 pb-24 pt-36 sm:px-8"><div className="mx-auto max-w-5xl"><section className="rounded-[2.75rem] bg-zinc-950 p-8 text-white sm:p-12"><div className="flex items-start justify-between gap-5"><Rocket/><ProfileProgressRing value={Math.round(completed/steps.length*100)} label="launch"/></div><p className="mt-5 text-xs font-bold uppercase tracking-[.2em] text-zinc-500">{profile.role} launch path</p><h1 className="mt-3 text-4xl font-semibold sm:text-6xl">Get JobiVerse-ready.</h1><p className="mt-4 text-zinc-400">{completed} of {steps.length} essential steps complete.</p><div className="mt-6 h-3 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-emerald-400" style={{ width: `${completed / steps.length * 100}%` }}/></div></section><div className="mt-6"><JoviGuide compact title="Your next best step" message={completed===steps.length?"Your essential setup is complete. Keep your workspace active and follow new opportunities.":`Complete ${steps.find(step=>!step.done)?.title.toLowerCase()??"your next step"} to keep your launch moving.`} href={steps.find(step=>!step.done)?.href} action="Continue setup" /></div><section className="mt-7 space-y-4">{steps.map((step) => <Link key={step.title} href={step.href} className="flex items-center gap-4 rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">{step.done ? <CheckCircle2 className="shrink-0 text-emerald-600"/> : <Circle className="shrink-0 text-zinc-300"/>}<div className="flex-1"><h2 className="font-bold">{step.title}</h2><p className="mt-1 text-sm text-zinc-500">{step.detail}</p></div><ArrowRight size={17}/></Link>)}</section></div></main>;
}
