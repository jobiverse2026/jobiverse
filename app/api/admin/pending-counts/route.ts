...
export async function GET() {
  try {
    ...
    const [
      ...
      supabase.from("notifications").select("id", { count: "exact", head: true }).eq("type", "payment_captured").is("read_at", null),
    ...
    return Response.json({
      ...
      "/admin/finance": (payouts.count ?? 0) + (paymentNotifications.count ?? 0),
