...
export async function POST(request: Request) {
  ...
  if (["payment.captured", "order.paid"].includes(event?.event) && payment?.order_id) {
    const { data: attempt } = await adminSupabase
      ...
      .select("id,user_id,target_type,target_id,local_order_id,amount")
    ...
    if (attempt) {
      await adminSupabase
        .from("payment_attempts")
        .update({ status: "captured", gateway_payment_id: payment.id })
        .eq("id", attempt.id);
      ...
      if (attempt.local_order_id) {
        await adminSupabase
          ...
          .eq("id", attempt.local_order_id)
          .eq("status", "pending_payment");
      }

      if (attempt.target_type === "resume_download") {
        await adminSupabase.from("resume_purchases").upsert(
          {
            user_id: attempt.user_id,
            template_id: attempt.target_id,
      ...
      if (attempt.target_type === "featured_listing") {
        ...
        if (service) {
          await adminSupabase.from("notifications").insert({
            ...
            message: `Your service "${service.title}" is now featured for 30 days.`,
  ...
  if (refund?.id && ["refund.processed", "refund.failed"].includes(event?.event)) {
    ...
    const { data: requestRow } = await adminSupabase
      ...
      .select("order_id,payment_attempt_id")
      .maybeSingle();

    if (requestRow && status === "processed") {
      await adminSupabase
        .from("marketplace_orders")
        .update({ status: "refunded", payout_status: "held" })
