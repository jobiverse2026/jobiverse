...
function destination(attempt:PaymentAttempt){if(attempt.target_type==="resume_download")return`/candidates/resume-builder?template=${encodeURIComponent(attempt.target_id)}&download=1`;if(attempt.target_type==="ai_resume_analysis")return"/candidates/resume-analysis?paid=1";if(attempt.target_type==="featured_listing")return"/earn-with-jobiverse/dashboard?featured=1";return`/payments/receipts/${attempt.id}`}
