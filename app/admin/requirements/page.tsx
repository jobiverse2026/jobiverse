...
export default async function RequirementsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  return <div className="space-y-8">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Requirements" value={(requirements??[]).length}/><Metric label="Open / active" value={(requirements??[]).filter((item)=>!["Closed","Cancelled","On Hold"].includes(item.status)).length}/><Metric label="JobiVerse team queue" value={(requirements??[]).filter((item)=>item.hiring_team_requested).length}/><Metric label="Published jobs" value={(requirements??[]).filter((item)=>item.is_public).length}/></section>
