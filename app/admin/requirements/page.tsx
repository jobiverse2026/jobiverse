...
const knownStatuses = ["Open", "Sourcing", "Interview", "Offer", "Joined", "Closed", "On Hold", "Cancelled"];

export default async function RequirementsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  await requireRole(["admin"]);
  const { q = "", status = "all" } = await searchParams;
  const [{ data: requirements, error }, { data: companies }, { data: recruiters }] = await Promise.all([
    adminSupabase.from("requirements").select("id,company_id,assigned_recruiter,job_title,department,experience,vacancies,location,priority,status,hiring_team_requested,is_public,created_at").eq("hiring_team_requested", true).order("created_at", { ascending: false }).limit(300),
  ...
  const rows = (requirements ?? []).filter((requirement) => {
    ...
    return (status === "all" || requirement.status === status) && (!normalized || [requirement.job_title, requirement.department, requirement.location, company?.company_name].some((value) => value?.toLowerCase().includes(normalized)));
  });
  const statusOptions = [...new Set([...(requirements ?? []).map((item) => item.status), ...knownStatuses])];

  return <div className="space-y-8">
    <section className="rounded-[2.5rem] bg-zinc-950 p-8 text-white sm:p-10"><BriefcaseBusiness/><p className="mt-5 text-xs font-bold uppercase tracking-[.18em] text-zinc-500">JobiVerse hiring operations</p><h1 className="mt-3 text-4xl font-bold">Assigned Requirements</h1><p className="mt-3 text-zinc-400">Only employer mandates where JobiVerse Hiring Team support was requested appear here. All other requirements stay with the employer workspace.</p></section>
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="JobiVerse queue" value={(requirements??[]).length}/><Metric label="Open / active" value={(requirements??[]).filter((item)=>!["Closed","Cancelled","On Hold"].includes(item.status)).length}/><Metric label="Published jobs" value={(requirements??[]).filter((item)=>item.is_public).length}/><Metric label="Filtered results" value={rows.length}/></section>
    <div className="rounded-2xl border border-violet-200 bg-violet-50 p-4 text-sm font-semibold text-violet-800"><span className="inline-flex items-center gap-2"><UsersRound size={17}/>Admin can submit JobiVerse-sourced candidates here. Marketplace visibility and requirement status are controlled by the employer.</span></div>
    <form className="grid gap-3 rounded-3xl border border-zinc-200 bg-white p-5 md:grid-cols-[1fr_220px_auto]"><input name="q" defaultValue={q} placeholder="Search role, company, department or location" className="h-12 rounded-xl border border-zinc-200 bg-zinc-50 px-4 outline-none focus:border-zinc-500"/><select name="status" defaultValue={status} className="h-12 rounded-xl border border-zinc-200 bg-zinc-50 px-4"><option value="all">All statuses</option>{statusOptions.map((item)=><option key={item} value={item}>{item}</option>)}</select><button className="cursor-pointer rounded-xl bg-zinc-950 px-6 font-semibold text-white">Apply filters</button></form>
