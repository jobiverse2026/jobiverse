...
export function RazorpayPaymentButton({ targetType, targetId, label = "Pay Securely" }: { targetType: PaymentTarget; targetId: string; label?: string }) {
  ...
  async function pay() {
    ...
    try {
      ...
      if (order.alreadyPaid) { window.location.href = order.redirectUrl; return; }
      const checkout = new window.Razorpay!({
        key: order.keyId, amount: order.amount, currency: order.currency, name: "JobiVerse", description: order.title, order_id: order.gatewayOrderId, theme: { color: "#18181b" },
