...
export default function ResumeAnalyzerClient({ hasResume, enabled }: { hasResume: boolean; enabled: boolean }) {
  ...
  return <div className="rounded-[2rem] border border-white bg-white/90 p-7 shadow-[0_25px_80px_-45px_rgba(0,0,0,.55)] sm:p-9">
    ...
    <button type="button" onClick={run} disabled={!enabled || !hasResume || loading} className="mt-7 inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-black via-zinc-800 to-zinc-600 px-6 py-4 font-semibold text-white shadow-lg disabled:cursor-not-allowed disabled:opacity-45">{!enabled ? <Clock3 size={19} /> : loading ? <LoaderCircle className="animate-spin" size={19} /> : <Sparkles size={19} />}{!enabled ? "Coming Soon" : loading ? "Analyzing your CV..." : "Run AI Analysis"}</button>
