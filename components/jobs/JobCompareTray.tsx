...
export function JobCompareTray() {
  ...
  return <aside aria-label="Selected jobs comparison" className="fixed bottom-4 left-1/2 z-50 w-[min(94vw,1020px)] -translate-x-1/2 overflow-hidden rounded-[1.5rem] border border-white/10 bg-zinc-950 text-white shadow-[0_24px_80px_rgba(0,0,0,.38)]">
    ...
    <div className="grid gap-3 p-3 sm:grid-cols-2 lg:grid-cols-[repeat(3,minmax(0,1fr))_auto] lg:items-stretch">
      {jobs.map((job, index) => <div key={job.key} className="flex min-w-0 items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[.07] px-3 py-2.5">
        <div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wider text-violet-300">Selection {index + 1}</p><p className="truncate text-sm font-bold" title={job.title}>{job.title}</p><p className="truncate text-[11px] text-zinc-400" title={job.company}>{job.company}</p></div>
