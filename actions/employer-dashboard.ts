...
export async function getEmployerDashboardData(){
  ...
  const [{data:company},{data:ownedRequirements,error}]=await Promise.all([
    adminSupabase.from("companies").select("id,recruiter_seat_limit,employer_seat_limit").eq("owner_id",user.id).maybeSingle(),
  ...
  const [employerMembers,employerInvites,recruiterMembers,recruiterInvites]=company ? await Promise.all([
    adminSupabase.from("employer_team_members").select("id",{count:"exact",head:true}).eq("company_id",company.id).eq("role","employer").eq("status","active"),
    adminSupabase.from("employer_team_invitations").select("id",{count:"exact",head:true}).eq("company_id",company.id).eq("role","employer").eq("status","pending").gt("expires_at",new Date().toISOString()),
    adminSupabase.from("employer_team_members").select("id",{count:"exact",head:true}).eq("company_id",company.id).eq("role","recruiter").eq("status","active"),
    adminSupabase.from("employer_team_invitations").select("id",{count:"exact",head:true}).eq("company_id",company.id).eq("role","recruiter").eq("status","pending").gt("expires_at",new Date().toISOString()),
